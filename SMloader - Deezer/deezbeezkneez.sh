...
# Gloabal Variables
	ECHO="echo -e"
	SMLOADER="~/bin/SMLoadr" # Path to SMloader tool
	OUTPUT="~/files/OMG_share/Music/" # Path to output folder
...
mir () {
	...
	sleep 5
	mirror=$( seq $x | parallel -j4 "$SMLOADER -q "FLAC" -d "single" -p "$OUTPUT" "https://www.deezer.com/us/ar/us/artist/{}" ") 
	$ECHO "$mirror" | $LOG
	$ECHO "" | $LOG
	$ECHO "" | $LOG
}
